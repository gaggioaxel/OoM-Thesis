...
\subsection{Normalize the distance from the baricenter}
As a first step, we will calculate the $x$, $y$, and $z$ coordinates of the skeleton's baricenter for each time step. \\
If we represent $n$ as the number of 20 joints, the coordinates of the baricenter are obtained by taking the average of the coordinates of these 20 joints, as we can see in the formula:


\begin{equation}
    \textit{Baricenter} (x, y, z) = \left(\frac{1}{n} \sum_{i=1}^{n} x_i, \frac{1}{n} \sum_{i=1}^{n} y_i, \frac{1}{n} \sum_{i=1}^{n} z_i\right)
    \label{formula:baricentro}
\end{equation}
    
Subsequently, we calculate the distance between each joint and the baricenter, at each time step, using the Euclidean distance in Formula \ref{formula:distance}. \\
These distances from the baricenter will be normalized for each joint, in order to obtain normalized time series between 0 and 1.
...
The choice to calculate the distances between joints and the baricenter for each sample plays a crucial role in ensuring the robustness of our ML approach to variations in scale.
