...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[every node/.style={circle, draw, minimum size=0.7cm}]
        % Livello superiore
        \node (A) at (3,0) [fill=red!60] {a};
        \node (B) at (3,-3) [fill=red!60] {b};
        
        % Livello inferiore
        \node (C) at (5,0) [fill=blue!60] {c};
        \node (D) at (5,-1.5) [fill=blue!60] {d};
        \node (E) at (5,-3) [fill=blue!60] {e};
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.45\textwidth}
        ...
        \begin{tikzpicture}[every node/.style={circle, draw, minimum size=0.7cm}]
            % Livello superiore
            \node (A) at (3,0) [fill=red!60] {a};
            \node (B) at (3,-1.5) [fill=red!60] {b};
            \node (C) at (3,-3) [fill=red!60] {c};

            % Livello inferiore
            \node (D) at (5,0) [fill=blue!60] {d};
            \node (E) at (5,-1.5) [fill=blue!60] {e};
            \node (F) at (5,-3) [fill=blue!60] {f};
...
Evaluation metrics in machine learning are fundamental tools for measuring the effectiveness and accuracy of predictive models.
...
The \textbf{confusion matrix} is an essential tool used in the evaluation of classification models in machine learning.
...
The \textbf{Receiver Operating Characteristic} (ROC) curve  is a fundamental tool in machine learning and statistics for evaluating and visualizing the performance of binary classification models. 
...
\begin{itemize}
  ...
  \begin{equation}
    \textit{False Positive Rate} = \frac{\text{FP}}{\text{FP + TN}}
...
\end{enumerate}
A random classifier would produce a diagonal line from (0,0) to (1,1), so a good model should have an ROC curve that is above this line.
\begin{figure}[H]
  \centering  
    \includegraphics[width=0.75\linewidth]{graphics/ROC curve.png}
    \caption{ROC Curve}
    \label{fig:ROC}
\end{figure}
...
ROC curves provide a visual depiction of a model's trade-off between \textit{Sensitivity} and \textit{Specificity} (1 - false positive rate) across different classification thresholds.