...
In the example shown in Figure \ref{}, the objective is to determine whether to grant a loan based on two pieces of information: 
the applicant's monthly income and the requested loan amount. The second figure illustrates the decision tree that has been trained on this dataset.
The underlying principle of the random tree is that at every non-terminal node, if there exist samples that need further separation, the following algorithm is applied: 
\begin{enumerate}
